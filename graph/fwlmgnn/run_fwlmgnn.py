...
def plot_confusion_matrix(preds, labels, save=False):
    ...
    if save: np.savez(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-cm_data.npz", cm=cm)
    if save: plt.savefig(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-confusion_matrix.png")
...
def plot_sep_theta(preds, labels, dataset, save=False):
    ...
    plt.grid()
    if save: np.savez(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-sep_vs_theta_data.npz", theta_bins=theta_bins, sep_powers=sep_powers)
    if save: plt.savefig(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-accuracy_vs_theta.png")
    if not save: plt.show()
...
def plot_sep_mom(preds, labels, dataset, save=False):
    ...
    plt.grid()
    if save: np.savez(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-sep_vs_momentum_data.npz", mom_bins=mom_bins, sep_powers=sep_powers)
    if save: plt.savefig(f"{os.path.dirname(args.model_input)}/{os.path.basename(args.model_input).replace('.pt', '')}-accuracy_vs_momentum.png")
    if not save: plt.show()
